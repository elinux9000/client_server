...
FILES_TO_CHECK := rcserver.c client.c rcserver.h client.h common.h common.c
...
rcserver: rcserver.o common.o steque.o
	$(CC) -o $@ $(CFLAGS) $(ASAN_FLAGS) $(CURL_CFLAGS) $^ $(LDFLAGS) $(CURL_LIBS) $(ASAN_LIBS)

client: client.o common.o steque.o
	$(CC) -o $@ $(CFLAGS) $(ASAN_FLAGS) $^ $(LDFLAGS) $(ASAN_LIBS)

rcserver_noasan: rcserver_noasan.o common_noasan.o steque_noasan.o
	$(CC) -o $@ $(CFLAGS) $(CURL_CFLAGS) $^ $(LDFLAGS) $(CURL_LIBS)

client_noasan: client_noasan.o common_noasan.o steque_noasan.o
	$(CC) -o $@ $(CFLAGS) $^ $(LDFLAGS)
...
fix:                                                                                                
	mycheckpatch.pl -fix-inplace -terse -file -no-tree --max-line-length=120 $(FILES_TO_CHECK) 
                                                                                                    
style:                                                                                              
	astyle -t --style=linux -n $(FILES_TO_CHECK) 

check:
	mycheckpatch.pl -terse --show-types -file -no-tree --ignore AVOID_EXTERNS --ignore SYMBOLIC_PERMS --ignore CONST_STRUCT --ignore SSCANF_TO_KSTRTO --ignore BRACES --max-line-length=120 $(FILES_TO_CHECK)
